...
class RDialog 
	...
	def calendar(text="Select a Date", height=0, width=0, day=Date.today.mday(), month=Date.today.mon(), year=Date.today.year())
		...
		command = option_array + "--calendar \"" + text.to_s + 
	...
	def msgbox(text="Text Goes Here", height=0, width=0)
		command = options_array + ["--msgbox", text.to_s,
                height.to_i.to_s, width.to_i.to_s]
		success = system(*command)
		return success
	end
	...
	def yesno(text="Please enter some text", height=0, width=0)
		command = options_array + ['--inputbox', text.to_s,
                height.to_i.to_s, width.to_i.to_s]
	...
	def options_array()
		unless @path_to_dialog
			...
			oarray = [@path_to_dialog]
		end
		
		oarray += ['--aspect', aspect]  if @aspect
    oarray += ['--beep']  if @beep
    oarray += ['--begin', @boxbegin[0], @boxbegin[1]]  if @boxbegin
    oarray += ['--backtitle', @backtitle]  if @backtitle
    oarray += ['--item-help']  if @itemhelp
		oarray += ['--sleep ', @sleep.to_i]  if @sleep
		oarray += ['--tab-correct']  if @tabcorrect
		oarray += ['--tab-len', @tablen.to_i]  if @tablen
		oarray += ['--title', @title.to_s]  if @title
    oarray += ['--nocancel']  if @nocancel
